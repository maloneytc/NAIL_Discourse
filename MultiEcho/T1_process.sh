...
#printStatus "Make mid-sagital image"
#imageName=${T1Dir}/$(basename $(dirname $(dirname ${inT1}))).png
#if [ ! -e ${imageName} ]; then
#    slicer ${inT1} -x .48 ${imageName}
#fi
...
if [ ! -e ${T1Dir}/seg_pve_2_thr95_eroded_masked.nii.gz ]; then
    ...
    flirt -ref ${T1Dir}/anat_ss.nii.gz -in ${FSLDIR}/data/standard/tissuepriors/avg152T1_white.img -out ${T1Dir}/avg152T1_white_inHighres -applyxfm -init ${T1Dir}/standard2highres.mat >>${stdOut} 2>>${stdErr}
    flirt -ref ${T1Dir}/anat_ss.nii.gz -in ${FSLDIR}/data/standard/tissuepriors/avg152T1_csf.img -out ${T1Dir}/avg152T1_csf_inHighres -applyxfm -init ${T1Dir}/standard2highres.mat >>${stdOut} 2>>${stdErr}
